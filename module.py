...
def faceDetector(image, gray, Draw=True):
    ...
    for face in faces:
        ...
        if Draw == True:
            cv.rectangle(image, cordFace1, cordFace2, WHITE, 2)
    return image, face
...
def EyeTracking(image, gray, eyePoints):
    ...
    centerPart = thresholdEye[0:height, 0]
    leftPart = thresholdEye[0:height, divPart:width]
    ...
    if pos == "Right":
        print("user memilih ke kanan")
    if pos == "Left":
        print("user memilih ke kiri")
